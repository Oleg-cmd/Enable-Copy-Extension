...
function renderWhitelist() {
  ...
  if (currentWhitelist.length === 0) {
    listElement.innerHTML =
      '<li style="font-style: italic;">No domains whitelisted.</li>';
    return;
  }
  [...currentWhitelist].sort().forEach((domain) => {
    const li = document.createElement("li");
    li.innerHTML = `<span style="word-break: break-all;">${domain}</span>`;
...
async function renderOcrLangs() {
  ...
  try {
    ...
    langList.innerHTML = "<p style='color:red;'>Error loading languages.</p>";
  }
}
